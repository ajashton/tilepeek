...
public:
    ...
    //
    // Implementations must be safe to call from non-GUI threads: the returned
    // QImage can then be converted to a QPixmap on the UI thread.
    virtual std::optional<QImage> tileAt(int zoom, int x, int y) = 0;
    ...
    virtual std::optional<QImage> tileAtSize(int zoom, int x, int y, int /*size*/)
    {
